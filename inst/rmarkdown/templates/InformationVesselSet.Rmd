...
-   Lances com mas de una posición (lon,lat):

```{r, ,echo=FALSE, results="show", comment=NA}
VesselSet <- paste(Base_f1$buque, Base_f1$lance, sep = "-")
L_VesselSet <- unique(VesselSet)
for(t in L_VesselSet){
  Base_Lance <- Base_f1[VesselSet == t, c("lon", "lat")]
...
-   Ubicación de lances correctos :
```{r, ,echo=FALSE, results="show", comment=NA}
par(mar = rep(0,4))
LANCE = paste(Base_f1$buque, Base_f1$lance, sep = "-")

pointMap <- point_area(x = Base_f1$lon,
                       y = Base_f1$lat, 
                       label = LANCE, 
                       catg = 0,
                       plot = T, col = 2)
...
if(is.na(stock)){
  ...
  Base_f2 = subset(x = Base, subset = Base$sp %in% SpTarget)
    ...
    rg.lat <- .getLati(sp = sp, stock = stock)
    Base_f2 = Base_f2[Base$sp %in% SpTarget,]
...
## Ubicación de lances para "SpTarget" :

```{r, ,echo=FALSE, results="show", comment=NA}
par(mar = rep(0,4))
LANCE = paste(Base_f3$buque, Base_f3$lance, sep = "-")

pointMap <- point_area(x = Base_f3$lon,
                       y = Base_f3$lat, 
                       label = LANCE, 
                       catg = 0,
                       plot = T, col = 2)
